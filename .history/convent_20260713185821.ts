// function — это наша мини-машинка, которая принимает данные и выдает результат.
// Мы строго говорим: amount (сумма) и rate (курс) должны быть только числами (number).
function convertCurrency(amount: number, rate: number): string {
  // Считаем чистый результат умножения
